...
/**
 * Provides a GraphViz string for a given {@link ClassifierTree}.
 * Builds on {@link ClassifierTree#graph()}, but extends the tree representation to include:
 * <ul>
 *     <li>Percentage of instances per node</li>
 *     <li>Distribution of class values per node</li>
 *     <li>Print dates in a readable format for edges</li>
 * </ul>
 */
class J48GraphPrinter {
    ...
    private String createSplitNodeLabel(ClassifierTree node, Attribute classAttribute) {
        ...
        for (int i = 0; i < numClasses; i++) {
            ...
            text.append(": ").append(Utils.roundDouble(distribution.perClass(i), 2));
            if (i < numClasses - 1) {
                text.append("|");
            } else {
                text.append("}");
