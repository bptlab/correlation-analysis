 ...
 */
public class MyOneR extends AbstractClassifier implements
     ...
     */
    static class OneRRule implements Serializable, RevisionHandler {
        ...
        public int percentCorrect() {
            return (m_correct * 100) / m_numInst;
        }

        public String valuesCorrelatingTo(String classValue) {
            var text = new StringBuilder();
            text.append(m_attr.name() + ":\n");
            for (int v = 0; v < m_classifications.length; v++) {
                var actualClassValue = m_class.value(m_classifications[v]);
